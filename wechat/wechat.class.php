...
class core_lib_wechat
{
    ...
    public function checkAccessToken(){
        ...
        $api = 'https://api.weixin.qq.com/sns/auth?access_token='.$this->access_token.'&openid='.$this->openid;
        $result = $this->http_get($api);
	...
	*/
	...
	function resizejpg($imgsrc,$imgwidth,$imgheight,$time)
	{
		...
		$imgWidth = $imgwidth;
		$imgHeight = $imgheight;
		$imgsrc_1 = imagecreatefromjpeg($imgsrc);
		if($imgsrc_1===false){
			$imgsrc_1 = imagecreatefrompng($imgsrc);
		}
		$imgsrc=$imgsrc_1;
	...
	function access_token($db) {
		$ret = $db -> getRow("SELECT * FROM `ecs_weixin_config` WHERE `id` = 1");
		$appid = $ret['appid'];
		$appsecret = $ret['appsecret'];
		$dateline = $ret['dateline'];
		$time = time();
		if (($time - $dateline) > 2000) {
			$url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=$appid&secret=$appsecret";
			$ret_json = $this ->http_get($url);
			$ret = json_decode($ret_json);
			if ($ret -> access_token) {
				$db -> query("UPDATE `ecs_weixin_config` SET `access_token` = '$ret->access_token',`dateline` = '$time' WHERE `id` =1;");
			}
		}

	}
	//获取新token  甜心100开发
	function access_newtoken($db) {
		$ret = $db -> getRow("SELECT * FROM `ecs_weixin_config` WHERE `id` = 1");
		$appid = $ret['appid'];
		$appsecret = $ret['appsecret'];
		$dateline = $ret['dateline'];
